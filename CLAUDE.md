...
│   └── r500.html           # Кейс R500 — журнал сборки двигателя (заполнен, без реальных скринов)
└── assets/
...
`.placeholder` — заглушка на месте будущего скриншота (сейчас осталась только под мобильные скрины в секции 05 кейса R500):
```html
<div class="placeholder aspect-[9/19]">мобильный скриншот · подпись</div>
```
Пунктирная рамка, мягкий фон, mono-подпись по центру. Задаётся `aspect-*` снаружи под нужные пропорции; растягивается на 100% ширины контейнера.
...
#projects — превью кейсов (реальные обложки кейсов, hover accent-tint)
  01 / r500 → cases/r500.html (наверху намеренно — единственный кейс «в разработке прямо сейчас», обложка `.placeholder`)
  02 / edcheck → cases/edcheck.html
  03 / близкие → cases/blizkie.html
#about — о себе
...
### `cases/r500.html` (внутренний журнал сборки двигателя)

```
01 / контекст      — "Пришёл слесарем-сборщиком, увидел продуктовую проблему"
02 / проблема      — "Подпись одна, а рук — пять и больше"
03 / решение       — "Один журнал на все операции вместо одной подписи под всеми операциями"
04 / логи повторов — "Если ротор идёт по второму кругу, история идёт вместе с ним"
05 / архитектура   — "Обычный веб-стек и ноутбук вместо сервера"
06 / статус сейчас — "Тестируем в бою прямо в цеху"
07 / рефлексия     — "Задача решается не макетом, а распределением ответственности"
```

**Важные факты, не путать:**
- Двигатель **роторный, малой тяги** — не «авиадвигатель» и не «реактивный». Если где-то всплывёт «авиадвигатель»/«реактивный двигатель» применительно к R500 — это регресс, исправить на «роторный двигатель малой тяги».
- Роль пользователя в этом кейсе шире, чем Product designer: он единственный **IT-внедритель** на производстве — сам настраивает программу, сам собирает требования у испытателей, слесарей и руководителей смены, сам дорабатывает. В `dl`-мете и в разделе 03 это явно проговорено, не сокращать обратно до просто «Product designer».
- Статус — **активная разработка**, не завершённый проект: журнал раскатан на планшеты и тестируется в реальной работе цеха прямо сейчас, функционал продолжает дорабатываться по фидбеку. Не переписывать в прошедшее время («сделал», «раскатал и всё») — только настоящее/незавершённое.
- Клиент обезличен («промышленное производство»), метрик по внедрению нет намеренно — рано, инструмент ещё не устоялся. Не выдумывать цифры.
- Кейс стоит **первым** в списке на `index.html` (не по хронологии, а потому что «в разработке» — самое живое и обсуждаемое сейчас). См. «Предпочтения» ниже.

Реальные скриншоты подключены (5 из 18 — см. таблицу `assets/r500/` ниже; остальные однотипные шаги форм намеренно не показываем, они в гитигнорной `unused/`). Осталось два `.placeholder aspect-[9/19]` в секции 05 — под мобильные скриншоты, пользователь доснимет.
...
Скриншоты журнала R500.UR (тёмная тема продукта). В кейс включены **осознанно только 5 из 18** — остальные однотипные шаги форм лежат в `assets/r500/unused/` (в `.gitignore`, в репозиторий не попадают, не подключать):

| Файл | Где используется |
|------|-----------------|
| `history.png` | Обложка карточки на index + секция 03 (история: счётчики + лог операций) |
| `sborka_step-1.png` | Кавер кейс-страницы (начало операции «Сборка») |
| `remont_step-2.png` | Секция 03 — пример этапов внутри операции (шаг «Булава» в ремонте) |
| `gipoteza.png` | Секция 03 — экран «Гипотеза» (изменение правил видят все отделы) |
| `ispitania_step-1.jpg` | Секция 04 — испытания с панелью «История двигателя» (доказательство логов повторов) |

Все `<img>` скриншотов R500 идут с классом `border border-border` — тёмные скрины на кремовом фоне без рамки сливаются краями в тёмной теме сайта.
...
- Порядок кейсов на главной — не хронологический и не по значимости, а по «живости»: активный проект в разработке (R500) идёт первым, завершённые (Edcheck, Близкие) — за ним. Если появится новый активный проект, аналогично поставить его наверх и сдвинуть остальные
- R500: пользователь — единственный исполнитель и IT-внедритель, не просто дизайнер. Отражать это в роли/тексте кейса, не сокращать до «Product designer»
...
- [ ] R500: доснять 2 **мобильных** скриншота (вертикальные, с телефона: шаг формы + история) и заменить ими `.placeholder aspect-[9/19]` в секции «05 / архитектура» — они доказывают сценарий «планшеты заняты → вношу с телефона → синхронизируется везде»
- [ ] R500: когда появятся метрики внедрения или проект завершится — дополнить/переписать секцию «06 / статус сейчас» и пересмотреть, остаётся ли кейс первым в списке
